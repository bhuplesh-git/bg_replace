...
with gr.Blocks(css=custom_css) as iface:
    ...
    with gr.Row():
        ...
        with gr.Column(elem_id="params"):
            ...
            if developer_mode:
                with gr.Tab('Options'):
                    num_inference_steps = gr.Slider(minimum=1, maximum=300,value=30, step=1, label="Num Inference Steps")
                    num_images_per_prompt = gr.Slider(minimum=1, maximum=100,value=1, step=1, label="Batch Size - Uses More VRAM")
                    controlnet_conditioning_scale = gr.Slider(minimum=0.1,value=0.65, maximum=2.0, step=0.01, label="ControlNet Conditioning Scale")
                    guidance_scale = gr.Slider(minimum=1.0, maximum=30.0,value=10.0, step=0.5, label="Guidance Scale")
                    num_images_to_generate = gr.Slider(minimum=1,value=1, maximum=10000,step=1, label="Number of Images to Generate")
